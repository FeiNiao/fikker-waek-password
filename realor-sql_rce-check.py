...
def check(url):
    ...
    try:
        ...
        if reps.status_code == 200 and (len(a5) != 0 or len(a6) != 0 or len(a71) != 0 or len(a72) != 0):
            print("\033[0;32;40m[+] {} 疑似存在瑞友天翼远程sql写入shell漏洞！！！\033[0m".format(url))
            f = open("res.txt","a+",encoding="utf-8")
            f.write(urls)
            f.write("\n")
            f.close()
        else:
            print("\033[0;31;40m[-] 该版本不存在漏洞或访问失败 url: {}\033[0m".format(url))
    except Exception as e:
        print("\033[0;31;40m[-] 访问失败 url : {}\033[0m".format(url))
